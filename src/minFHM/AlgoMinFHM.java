...
/**
 * Created by Ziyun on 2016/11/25.
 *
 */
...
public class AlgoMinFHM {
	...
	List<List<minFHM.Itemset>> listItemsetsBySize = null;
...
//		System.out.println();
		...
		for(int i=0; i < itemset.length	&& i < listItemsetsBySize.size(); i++) {    // IMPORTANT -1
			List<minFHM.Itemset> list = listItemsetsBySize.get(i);
			if(list.size() > 0) {
				for(minFHM.Itemset itemsetInList : list) {
...
//		System.out.println();
		...
		if(itemset.length >= listItemsetsBySize.size()) {
			...
			while(i <= itemset.length) {
				listItemsetsBySize.add(new ArrayList<minFHM.Itemset>());
				i++;
			}
		}

		// add the itemset in the list of itemsets having the same size
		List<minFHM.Itemset> listToAdd = listItemsetsBySize.get(itemset.length);
		listToAdd.add(new minFHM.Itemset(itemset, utility, support));

		// REMOVE ALL SUPERSETS of the inserted itemset
		for(int i=itemset.length+1; i < listItemsetsBySize.size(); i++) {    // IMPORTANT -1
			List<minFHM.Itemset> list = listItemsetsBySize.get(i);
			if(list.size() > 0) {
				Iterator<minFHM.Itemset> iter = list.iterator();
				while (iter.hasNext()) {
					minFHM.Itemset itemset2 = (minFHM.Itemset) iter.next();
	...
	public void runAlgorithm(String input, String output, int minUtility) throws IOException {
		...
		listItemsetsBySize = new ArrayList<List<minFHM.Itemset>>();
		...
		List<minFHM.UtilityList> listOfUtilityLists = new ArrayList<minFHM.UtilityList>();
		// CREATE A MAP TO STORE THE UTILITY LIST FOR EACH ITEM.
		// Key : item    Value :  utility list associated to that item
		Map<Integer, minFHM.UtilityList> mapItemToUtilityList = new HashMap<Integer, minFHM.UtilityList>();
		...
		for(Integer item: mapItemToTWU.keySet()){
			...
			if(mapItemToTWU.get(item) >= minUtility && mapItemToUtility.get(item) < minUtility){
				...
				minFHM.UtilityList uList = new minFHM.UtilityList(item);
		...
		Collections.sort(listOfUtilityLists, new Comparator<minFHM.UtilityList>(){
			public int compare(minFHM.UtilityList o1, minFHM.UtilityList o2) {
		...
		try {
			...
			while ((thisLine = myInput.readLine()) != null) {
				...
				for(int i = 0; i< revisedTransaction.size(); i++){
					...
					minFHM.UtilityList utilityListOfItem = mapItemToUtilityList.get(pair.item);

					// Add a new Element to the utility list of this item corresponding to this transaction
					minFHM.Element element = new minFHM.Element(tid, pair.utility, remainingUtility);
...
//		}
		...
		for(Map.Entry<Integer,Integer> entry : mapItemToUtility.entrySet()) {
		...
		for(List<minFHM.Itemset> listItemsets : listItemsetsBySize) {
			for(minFHM.Itemset itemset : listItemsets) {
				writeOut(itemset);
			}
		}
	 ...
	 * @throws IOException
	 */
	private void minfhm(int [] prefix, minFHM.UtilityList pUL, List<minFHM.UtilityList> ULs, int minUtility)
			throws IOException {

		// For each extension X of prefix P
		for(int i=0; i< ULs.size(); i++){
			minFHM.UtilityList X = ULs.get(i);
			...
			if(X.sumIutils + X.sumRutils >= minUtility){
				...
				List<minFHM.UtilityList> exULs = new ArrayList<minFHM.UtilityList>();
				// For each extension of p appearing
				// after X according to the ascending order
				for(int j=i+1; j < ULs.size(); j++){
					minFHM.UtilityList Y = ULs.get(j);
					...
					minFHM.UtilityList pXY = construct(pUL, X, Y, minUtility);
	 ...
	 */
	private minFHM.UtilityList construct(minFHM.UtilityList P, minFHM.UtilityList px, minFHM.UtilityList py, int minUtility) {
		// create an empy utility list for pXY
		minFHM.UtilityList pxyUL = new minFHM.UtilityList(py.item);
		...
		for(minFHM.Element ex : px.elements){
			// do a binary search to find element ey in py with tid = ex.tid
			minFHM.Element ey = findElementWithTID(py, ex.tid);
			if(ey == null){
				//== new optimization - LA-prune == /
				if(ENABLE_LA_PRUNE) {
			...
			if(P == null){
				...
				minFHM.Element eXY = new minFHM.Element(ex.tid, ex.iutils + ey.iutils, ey.rutils);
				// add the new element to the utility list of pXY
				pxyUL.addElement(eXY);

			}else{
				// find the element in the utility list of p wih the same tid
				minFHM.Element e = findElementWithTID(P, ex.tid);
				if(e != null){
					// Create new element
					minFHM.Element eXY = new minFHM.Element(ex.tid, ex.iutils + ey.iutils - e.iutils,
							ey.rutils);
					// add the new element to the utility list of pXY
					pxyUL.addElement(eXY);
				}
	 ...
	 */
	private minFHM.Element findElementWithTID(minFHM.UtilityList ulist, int tid){
		List<minFHM.Element> list = ulist.elements;
	 ...
	 *  the prefix to be writent o the output file
	 *  an item to be appended to the prefix
	 *  utility the utility of the prefix concatenated with the item
	 */
	private void writeOut(minFHM.Itemset itemset) throws IOException {
...
//		buffer.append(itemset.support);
	...
	public void printStats() throws IOException {
		...
		if(debug) {
			...
			for(Map.Entry<Integer, Map<Integer, Integer>> entry : mapFMAP.entrySet()) {
